...
$query = "select * from assegurança;";
$resultado = mysqli_query($connection, $query);
$ultimo_id_seguro = '';
?>

<div class="centrado registro" style="margin-top: 30px;">
<h2>Insertar datos en seguros</h2>
<table align="center">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><input type="date" name="inicio_<?php echo $contador; ?>" value="<?php echo $result[3] ?>" style="width: 100px;" readonly></td>
      <td><input type="date" name="fin_<?php echo $contador; ?>" value="<?php echo $result[4] ?>" style="width: 100px;"></td>
      <td><input type="text" name="destino_<?php echo $contador; ?>" value="<?php echo $result[5] ?>" style="width: 100px;" readonly></td>
      <td><input type="number" name="personas_<?php echo $contador; ?>" value="<?php echo $result[6] ?>" style="width: 80px;" readonly></td>
      <td><input type="submit" name="update_<?php echo $contador; ?>" class="boton" value="Guardar" style="width: 100%;"/></td>
    </tr>
    <?php
    $ultimo_id_seguro = $result[0];
    $ultimo_id_seguro++;
    $contador++;
    }
    ?>
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" name="add " method="post" enctype="multipart/form-data">
  <tr>
    <td><input type="text" name="id" value="<?php echo $ultimo_id_seguro; ?>" style="width: 30px;" readonly></td>
    <td><input type="text" name="companyia" style="width: 100px;"></td>
    <td><input type="date" name="inicio" min="1" max="5" style="width: 120px;"></td>
    <td><input type="date" name="fin" style="width: 120px;"></td>
    <td><input type="text" name="destino" style="width: 100px;"></td>
    <td><input type="number" name="personas" style="width: 80px;"></td>
  </tr>
  <tr>
...
if (isset($_POST["enviar"])) {
  ...
  $insert = "INSERT INTO `assegurança`(`ID`, `companyia`, `puntual`, `datainici`, `datafi`, `desti`, `persones`) 
  VALUES ($ultimo_id_seguro,'$companyia',1,'$inicio','$fin','$destino',$personas);";
  try {
    if (mysqli_query($connection, $insert)) {
        echo "<script>window.location.replace('Agregar_seguros.php');</script>";
    }
  } catch (Exception $ex) { ?>
...
for ($identificador=1; $identificador<1000; $identificador++) {
  if (isset($_POST["update_$identificador"])) {
    ...
    $fin = $_POST["fin_$identificador"];
    //hacer update de uno que ya estaba
    $update = "UPDATE `assegurança` 
    SET `datafi`='$fin'
    WHERE ID = $id;";
    try {
